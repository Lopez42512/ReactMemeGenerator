...
class MemeGenerator extends Component {
  ...
  render() {
    ...
    const selectNumber = fontOption.map(number => {
      return (
    ...
    let changeText;
    this.state.isTrue
      ? (changeText = "Change Font to White")
      : (changeText = "Change Font to Black");
    let getAllMemes = this.state.memeHolder.map(meme => (
      <img
    ...
    let meme;
    this.state.allMeme
      ? (meme = <div className="allMemeContainer">{getAllMemes}</div>)
      : (meme = <img src={this.state.imgUrl} alt="problem"></img>);
      ...
      <div>
        <div className="textcontainer">
          <div className="toprow">
            ...
            />
            <button onClick={this.handleClick} name="isTrue">
              {changeText}
            </button>
            <label>
              <select
          ...
          <div className="bottomrow">
            <button onClick={this.handleClick} name="prev">
              Previous Meme
            </button>
            <button onClick={this.handleClick} name="random">
              Random Meme
            </button>
            <button onClick={this.handleClick} name="next">
              Next Meme
            </button>
            <button onClick={this.handleClick} name="meme">
              Choose A Meme
            </button>
          </div>
        </div>
        <div style={{ fontSize: this.state.fontSize }} className="imgcontainer">
          {meme}
          <Draggable>
            <h2 style={{ color: color }}>{this.state.text1}</h2>
          </Draggable>
          <Draggable>
            <h2 style={{ top: "50%", color: color }}>{this.state.text2}</h2>
          </Draggable>
        </div>
      </div>
    );
